...
const ProblemSection = ({ link }) => {
  ...
  const styles = {
    ...
    link: {
      wordBreak: 'break-all', // Add this property to handle long URLs
      color: '#007BFF',
    },
    buttonsContainer: {
      display: 'flex',
    ...
    addButton: {
      ...
      cursor: 'pointer',
      fontSize: '16px',
      wordWrap: 'break-word', // Add this property to handle long button texts
    },
    deleteButton: {
      padding: '8px 12px',
      borderRadius: '6px',
  ...
  if (window.innerWidth <= 768) {
    ...
    styles.addButton.padding = '6px 10px';
    styles.deleteButton.padding = '6px 10px';
  }
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        <div style={{ display: "flex", flexDirection: "column", textAlign: "left" }}>
          ...
          {/* Link text removed, only the title is displayed */}
        </div>
